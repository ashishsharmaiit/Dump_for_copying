    ...
    try:
        ...
        for item in response['Items']:
            if item['port']['S'] != LOCAL_PORT:
                print(f"Peer IP: {item['ip']['S']}, Peer Port: {int(item['port']['S'])}")
...
def start_udp_chat(local_port, peer_ip, peer_port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(('', local_port))  # Binding to any available IP address on the system
    ...
    def send_messages():
        while True:
            ...
            print(f"Sending to {peer_ip}:{peer_port}: {message}")
...
if public_ip is not None:
    ...
    if peer_ip is not None and peer_port is not None:
        start_udp_chat(int(LOCAL_PORT), peer_ip, peer_port)
    else:
